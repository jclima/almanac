...
Welcome to the **Almanac** firmware. This guide outlines the hardware controls, navigation, and reading features of the device.
...
Almanac supports sending books from Calibre using the CrossPoint Reader device plugin.
...
- **Language**: Set the UI language. Almanac ships 31 translations: Arabic, Belarusian, Bosnian, Brazilian Portuguese, Catalan, Czech, Danish, Dutch, English, European Portuguese, Finnish, French, German, Hebrew, Hungarian, Indonesian, Italian, Kazakh, Lithuanian, Norwegian Bokmål, Polish, Romanian, Russian, Slovak, Slovenian, Spanish, Swedish, Turkish, Ukrainian, Valencian, and Vietnamese. Arabic and Hebrew render right-to-left.
...
Almanac supports saving multiple OPDS servers and switching between them when browsing catalogs.
...
2. In a browser, open `http://<device-ip>/settings` or `http://almanac.local`.
...
Almanac can sync reading progress with KOReader-compatible sync servers.
It also interoperates with KOReader apps/devices when they use the same server and credentials.

##### Option A: CrossPoint Sync Server (`sync.crosspointreader.com`, default)

When **Sync Server URL** is left empty, Almanac uses the free CrossPoint sync server at `https://sync.crosspointreader.com`. It speaks the standard KOReader sync protocol (so KOReader apps can use it too). Almanac records page starts as chapter-content offsets and sends the corresponding standard KOReader XPath, so devices with different fonts or layouts can return to the same text.

1. On each Almanac device:

   - Go to **Settings -> System -> KOReader Sync**.

   - Set **Username** and **Password** (enter the plain password; Almanac computes MD5 internally, and use the same values on all devices).
...
3. Register a user once.
   Almanac authenticates against KOReader Sync (`koreader/kosync`) using an MD5 key, so register using the MD5 of your password:
...
| Mode               | Behavior                                                                                                                     |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| **Dark** (default) | The Almanac logo on a dark background.                                                                                    |
| **Light**          | The Almanac logo on a white background.                                                                                   |
...
Almanac supports loading additional fonts from the SD card, extending beyond the two built-in families (Noto Serif, Noto Sans). Custom fonts can include extended Unicode coverage, enabling CJK (Chinese, Japanese, Korean) and other scripts.
...
Almanac renders text using the following Unicode character blocks, enabling support for a wide range of languages:
...
**Crash reports on SD card:** After a crash, Almanac automatically saves a crash report to the SD card (no USB connection needed). Check the root of the SD card for a crash log file and include it with any bug report.
