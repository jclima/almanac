...
1. **HTML Headers** (generated by `scripts/build_html.py`):
   ...
   - **Source**: HTML/JS alongside the generated headers in `src/network/html/` (there is no `data/` directory; `build_html.py` walks `src/`)
   - **Triggered**: During PlatformIO `pre:` build step
   - **To modify**: Edit the `.html`/`.js` in `src/network/html/`, not the `.generated.h`
