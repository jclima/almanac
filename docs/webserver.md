...
- The mDNS fallback URL, usually `http://almanac.local/`
...
3. Open the URL shown on the reader. `http://almanac.local/` is preferred
...
2. Prefer `almanac.local` when available, but keep the displayed IP address as a fallback.
