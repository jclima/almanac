...
AlmanacWebServer* wsInstance = nullptr;
...
AlmanacWebServer::AlmanacWebServer() {}

AlmanacWebServer::~AlmanacWebServer() { stop(); }

void AlmanacWebServer::begin() {
  if (running) {
  ...
  wsInstance = const_cast<AlmanacWebServer*>(this);
  wsServer->begin();
...
void AlmanacWebServer::abortWsUpload(const char* tag) {
...
void AlmanacWebServer::handleClient() {
...
AlmanacWebServer::WsUploadStatus AlmanacWebServer::getWsUploadStatus() const {
...
void AlmanacWebServer::handleRoot() const {
  sendHtmlContent(server.get(), HomePageHtml, sizeof(HomePageHtml));
  LOG_DBG("WEB", "Served root page");
}

void AlmanacWebServer::handleJszip() const {
  server->sendHeader("Content-Encoding", "gzip");
  server->send_P(200, "application/javascript", jszip_minJs, jszip_minJsCompressedSize);
  LOG_DBG("WEB", "Served jszip.min.js");
}

void AlmanacWebServer::handleNotFound() const {
...
void AlmanacWebServer::handleStatus() const {
  // Get correct IP based on AP vs STA mode
  const String ipAddr = apMode ? WiFi.softAPIP().toString() : WiFi.localIP().toString();

  JsonDocument doc;
  doc["version"] = ALMANAC_VERSION;
  doc["ip"] = ipAddr;
...
void AlmanacWebServer::scanFiles(const char* path, const std::function<void(FileInfo)>& callback) const {
...
bool AlmanacWebServer::isEpubFile(const String& filename) const { return FsHelpers::hasEpubExtension(filename); }

void AlmanacWebServer::handleFileList() const { sendHtmlContent(server.get(), FilesPageHtml, sizeof(FilesPageHtml)); }

void AlmanacWebServer::handleFileListData() const {
...
void AlmanacWebServer::handleDownload() const {
  if (!server->hasArg("path")) {
...
static bool flushUploadBuffer(AlmanacWebServer::UploadState& state) {
...
void AlmanacWebServer::handleUpload(UploadState& state) const {
...
void AlmanacWebServer::handleUploadPost(UploadState& state) const {
  if (state.success) {
...
void AlmanacWebServer::handleCreateFolder() const {
...
void AlmanacWebServer::handleRename() const {
...
void AlmanacWebServer::handleMove() const {
...
void AlmanacWebServer::handleDelete() const {
...
void AlmanacWebServer::handleSettingsPage() const {
  sendHtmlContent(server.get(), SettingsPageHtml, sizeof(SettingsPageHtml));
  LOG_DBG("WEB", "Served settings page");
}

void AlmanacWebServer::handleGetSettings() const {
...
void AlmanacWebServer::handlePostSettings() {
...
void AlmanacWebServer::handleGetOpdsServers() const {
...
void AlmanacWebServer::handlePostOpdsServer() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void AlmanacWebServer::handleDeleteOpdsServer() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void AlmanacWebServer::handleGetWifiNetworks() const {
...
void AlmanacWebServer::handlePostWifiNetwork() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
// Uses POST (not HTTP DELETE) because ESP32 WebServer doesn't support DELETE with body.
void AlmanacWebServer::handleDeleteWifiNetwork() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void AlmanacWebServer::wsEventCallback(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  if (wsInstance) {
...
void AlmanacWebServer::onWebSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
...
void AlmanacWebServer::handleFontsPage() const {
  sendHtmlContent(server.get(), FontsPageHtml, sizeof(FontsPageHtml));
  LOG_DBG("WEB", "Served fonts page");
}

void AlmanacWebServer::handleFontList() const {
...
void AlmanacWebServer::handleFontUploadData() {
...
void AlmanacWebServer::handleFontUpload() {
  if (fontUpload.valid) {
...
void AlmanacWebServer::handleFontDelete() {
